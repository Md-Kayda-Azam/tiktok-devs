...
import { hashPassowrd, varifyPassword } from "../utility/hash.js";
...
import { isEmail, usernameValidate } from "../utility/Validate.js";
...
export const register = async (req, res, next) => {
  try {
    ...
    // form validate
    if (!usernameValidate(username) || !email || !password || !gender) {
...
export const login = async (req, res, next) => {
  try {
    ...
    } else if (!varifyPassword(password, loggedInUser.password)) {
...
export const forgotPassword = async (req, res, next) => {
  try {
    ...
    if (!user) {
      ...
      // check code
      if (checkCode) {
        sentForgotPassowrdcode = getRandom(555555, 333333);
      }
